...
df = pd.read_csv('/Users/minhnguyetnguyen/Documents/AISOUP/AutoML/Database/DB5/db5mod2.csv')

#cols = ['age', 'menopause', 'tumor_size', 'inv_nodes','node_caps','breast', 'breast_quad','irradiat', 'Class']
#df[cols] = df[cols].apply(LabelEncoder().fit_transform)
X = np.array(df.drop(['Disease_state'], axis=1))  # input
y1 = np.array(df['Disease_state'])
